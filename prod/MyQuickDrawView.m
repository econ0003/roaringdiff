...
- (void)scrollToLine:(int)lineno
{
	...
	if (denominator > 0) {
		 ...
		 *
		 * The 1 extra line is so that we show the previous line, which is important when we're jumping to an 'add'
		 * difference, becuase there is no line number on the add, and it appears we've jumped into the middle of
		 * a diff.  this helps clarify.
		 */
...
- (void)drawRect:(NSRect)rect
{
	...
	/* Paint the background a soft gray */
	RGBColor col;
	...
	if (rd_prefs_get_show_linenos()) {
		...
		if (col.red < 52428 && col.green < 52428 && col.blue < 52428) {	/* check if we could overflow */
...
qd_render_add(qd_render_t *r, rd_diff_t *d, int lineno)
{
	...
	// We're not changing a line at <lineno>, we're inserting after it.  The line at <lineno> still needs to be drawn!
	r->v->effective_line++;
