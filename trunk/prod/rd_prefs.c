...
typedef struct {
	...
	int font_size;				/**< XXX: We'll want a more fully feature font selection tool here later. */
...
/**
 * When comparing directories, some file names we want to skip. (e.g., CVS, .cvs, .svn, tags, .o, .out)
 */
typedef struct {
	char regexp[64];			/**< XXX: */
} rd_dir_skip_file_t;
